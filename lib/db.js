  ...
  try {
    ...
    await db`
      CREATE TABLE IF NOT EXISTS documents (
        ...
        type VARCHAR(50) NOT NULL CHECK (type IN ('manual', 'specs', 'diagram', 'firmware', 'guide')),
        category VARCHAR(50) NOT NULL CHECK (category IN ('camara', 'audio', 'iluminacion', 'video', 'streaming', 'accesorio')),
  ...
  try {
    const [document] = await db`
      INSERT INTO documents (
        name, type, category, brand, model, 
        file_name, file_url, file_size, file_type,
        keywords, description, uploaded_by
      ) 
      VALUES (
        ${documentData.name},
        ${documentData.type},
