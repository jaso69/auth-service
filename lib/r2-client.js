...
static async generateDownloadURL(fileKey, originalFileName = null, expiresIn = 3600) {
  ...
  try {
    // Usar el nombre original del archivo o extraerlo de la key
    const filename = originalFileName || fileKey.split('/').pop();
    
    // Limpiar el nombre de archivo (remover espacios y caracteres especiales)
    const cleanFilename = filename.replace(/%20/g, ' ').trim();
    
    console.log('📁 Nombre de archivo limpio:', cleanFilename);

    const command = new GetObjectCommand({
      Bucket: R2_CONFIG.bucketName,
      Key: fileKey,
      // ✅ FORZAR el nombre de archivo correcto en la descarga
    ...
    console.log('✅ URL firmada generada con nombre correcto');
