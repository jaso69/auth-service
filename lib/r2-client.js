...
static async uploadDocument(file, documentId, options = {}) {
  try {
    ...
    // IMPORTANTE: Obtener el buffer correctamente
    let fileBody;
    let contentType = file.type || file.mimetype || 'application/octet-stream';
    ...
    if (file.buffer instanceof Buffer) {
      ...
      // Si file.buffer existe pero no es Buffer, intentar convertir
      fileBody = Buffer.from(file.buffer);
    } else {
      throw new Error('No se pudo obtener el contenido del archivo como Buffer');
    }
    ...
    console.log('📦 Buffer info:', {
      isBuffer: Buffer.isBuffer(fileBody),
      size: fileBody.length,
      first10Bytes: fileBody.slice(0, 10).toString('hex')
    });

    // Para PDFs, verificar que el buffer empiece con %PDF
    if (contentType === 'application/pdf') {
      const pdfHeader = fileBody.slice(0, 4).toString('ascii');
      if (pdfHeader !== '%PDF') {
        console.warn('⚠️ Warning: El archivo no parece ser un PDF válido');
    ...
    if (file.size <= MULTIPART_THRESHOLD) {
      result = await this._uploadSimple(
        { ...file, buffer: fileBody, mimetype: contentType }, 
        documentId, 
        key, 
        metadata
      );
    } else {
      result = await this._uploadMultipart(
        { ...file, buffer: fileBody, mimetype: contentType }, 
        documentId, 
        key, 
        metadata, 
 ...
 static async _uploadSimple(file, documentId, key, metadata = {}) {
  ...
  // Asegurarse de que el buffer es válido
  if (!Buffer.isBuffer(file.buffer)) {
    throw new Error('File buffer is not a valid Buffer in _uploadSimple');
  }
  
  const command = new PutObjectCommand({
    Bucket: R2_CONFIG.bucketName,
    Key: key,
    Body: file.buffer, // Buffer directamente
    ContentType: file.mimetype || 'application/octet-stream',
    ContentLength: file.buffer.length, // Usar length del buffer
    Metadata: {
  ...
  console.log('✅ Upload response:', response.$metadata.httpStatusCode);
 ...
 static async generateUploadURL(documentId, fileType, options = {}) {
   ...
   */
  // En tu r2-client.js - MODIFICA el método generateDownloadURL:

// En tu r2-client.js - MODIFICA el método generateDownloadURL:

// En tu r2-client.js - MODIFICA el método generateDownloadURL:

static async generateDownloadURL(fileKey, originalFileName = null, expiresIn = 3600) {
  console.log('📄 generateDownloadURL llamado');
  ...
  try {
    ...
    const cleanFilename = filename.replace(/%20/g, ' ').trim();

    const command = new GetObjectCommand({
      Bucket: R2_CONFIG.bucketName,
      Key: fileKey,
      ResponseContentDisposition: `attachment; filename="${cleanFilename}"`,
      ResponseContentType: contentType // Usar el tipo correcto
    });
    ...
    console.log('✅ URL firmada generada con Content-Type:', contentType);
    ...
    console.error('❌ Error en generateDownloadURL:', error);
    return {
      success: false,
      error: error.message
    };
