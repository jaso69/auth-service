...
export class R2Client {
  ...
  static async uploadDocument(file, documentId, options = {}) {
    try {
      ...
      // Para archivos pequeños (< 5MB), usar upload simple
      if (file.size <= MULTIPART_THRESHOLD) {
        return await this._uploadSimple(file, documentId, key, metadata);
      } else {
        // Para archivos grandes, usar upload multiparte
        return await this._uploadMultipart(file, documentId, key, metadata, options);
      }
    } catch (error) {
